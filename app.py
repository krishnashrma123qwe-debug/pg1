...
# ========== FLASK SETUP ==========
app = Flask(__name__)
# New secret key every run → old sessions invalidated
...
def automation_rules():
    ...
    if temperature > 28 and not devices["ac"]:
        ...
        speak(msg); add_notification(msg)
...
def add_no_cache_headers(response):
    """Prevent browser caching so back button after logout doesn’t show UI."""
...
def set_fan_speed():
    ...
    speak(msg); add_notification(msg)
    print(f"Fan PWM set to {fan_speed}%")
...
def voice_command():
    ...
    if "light" in command:
        if "on" in command: devices["light"] = True; state="ON"; message="Turning on the light."
        elif "off" in command: devices["light"] = False; state="OFF"; message="Turning off the light."
        device = "light"
    elif "fan" in command:
        if "on" in command: devices["fan"] = True; state="ON"; message="Turning on the fan."
        elif "off" in command: devices["fan"] = False; state="OFF"; message="Turning off the fan."
        device = "fan"
    elif "ac" in command:
        if "on" in command: devices["ac"] = True; state="ON"; message="Turning on the AC."
        elif "off" in command: devices["ac"] = False; state="OFF"; message="Turning off the AC."
        device = "ac"

    if device:
        speak(message)
        add_notification(f"🎙️ Voice command: {message}")
        return jsonify({"status": "success", "message": message, "device": device, "state": state})
...
def get_emergency_status():
    ...
    if detected:
        ...
        emergency_active["message"] = "🔥 Fire/Gas detected! All devices OFF. Alerting Fire Department!"
